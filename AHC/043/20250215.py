...
class Solver:
    ...
    def solve(self) -> Result:
        ...
        while len(self.actions) < self.T:
            ...
            if rail_count <= len(self.actions) * (100 / 800):
                self.build_nothing()
                self.money += income
                continue
            person_idx = 0
            for person_idx in self.non_station_person_list:
                if self.distance[person_idx] - 1 <= rail_count:
                    for ds in dist_list:
                        for de in dist_list:
                            s = (
                            ...
                            if (
                                ...
                                and 0 <= e[0] < self.N
                                and 0 <= e[1] < self.N
                                and distance(s, e) - 1 <= rail_count
                            ):
                                if (s, e) not in expect_income_sum:
                                    if (
                                        self.field.rail[s[0]][s[1]] != STATION
                                        and self.field.rail[e[0]][e[1]] != STATION
                                    ):
                                        expect_income_sum[(s, e)] = -COST_STATION * 2
                                    elif (
                                        self.field.rail[s[0]][s[1]] == STATION
                                        and self.field.rail[e[0]][e[1]] == STATION
                                    ):
                                        ...
                                        expect_income_sum[(s, e)] = -COST_STATION
                                expect_income_sum[(s, e)] += self.distance[
                                    person_idx
            ...
            if self.field.rail[end[0]][end[1]] != STATION:
                station_cnt += 1
            # 建築するメリットがあるかどうか
            if expect_income_sum[max_pair] < 0:
                self.build_nothing()
                self.money += income
                continue
