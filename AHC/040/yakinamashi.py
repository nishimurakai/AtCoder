...
for step in range(T):
    # print(f"# {calc_W_H(prdb)}")
    prdb, best_W, best_H = yakinamashi(prdb)
    # print(f"# {calc_W_H(prdb)}")
    # W_obs, H_obs, x_list_act, y_list_act = output_with_act(prdb)
    W_obs, H_obs = output(prdb)
    # print(f"# W_obs:{W_obs} H_obs:{H_obs} score:{W_obs + H_obs}")
    # w_p, h_p = W_obs/best_W, H_obs/best_H
    # _, _, x_list, y_list = calc_W_H(prdb)
    # plot_shapes(x_list_act, y_list_act, f"result_{step}.png")
    # plot_shapes(x_list, y_list, f"result_{step}_best.png")
