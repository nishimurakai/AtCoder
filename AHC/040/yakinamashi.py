...
def change_prdb(prdb):
    i = rng.integers(0, N)
    original_value = prdb[i]
    prdb[i] = (
        i,
        rng.integers(0, 2),
        ['U', 'L'][rng.integers(0, 2)],
        rng.integers(-1, i),
    )
    return prdb, original_value, i

def yakinamashi(prdb):
    ...
    while time.perf_counter() - t_start < t_final:
        yaki_cnt += 1
        new_prdb, original_value, idx = change_prdb(prdb)
        new_W, new_H, _, _ = calc_W_H(new_prdb)
        new_score = new_W + new_H
        time_passed = time.perf_counter() - start_time
        temp = start_time * np.power((end_time / start_time), time_passed)
        if new_score < best_score:
            ...
            prdb = new_prdb
            score = new_score
        elif rng.random() < np.exp((score - new_score) / temp):
            prdb = new_prdb
            score = new_score
        else:
            prdb[idx] = original_value
    print(f"# yaki_cnt: {yaki_cnt}")
...
idxs = np.arange(N)
rand_ints_0_1 = rng.integers(0, 2, size=N)
rand_chars = rng.choice(['U', 'L'], size=N)
rand_ints_neg1_i = [rng.integers(-1, i) for i in range(N)]

# best_prdbの作成
best_prdb = [
    (i, rand_ints_0_1[i], rand_chars[i], rand_ints_neg1_i[i])
    for i in idxs
]

for step in range(T):
    prdb, best_W, best_H = yakinamashi(best_prdb[:])
    # W_obs, H_obs = output(prdb)
    # if step  == 0:
    #     best_prdb = prdb
    #     best_W = W_obs
    #     best_H = H_obs
    # elif W_obs + H_obs < best_W + best_H:
    #     best_prdb = prdb
    #     best_W = W_obs
    #     best_H = H_obs
